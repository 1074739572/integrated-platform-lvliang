...
public class QTEtlFlow extends com.querydsl.sql.RelationalPathBase<TEtlFlow> {
	...
	public void addMetadata() {
		...
		addMetadata(flowDesp, ColumnMetadata.named("FLOW_DESP").withIndex(5).ofType(Types.VARCHAR));
		addMetadata(flowTplName, ColumnMetadata.named("FLOW_TPL_NAME").withIndex(6).ofType(Types.VARCHAR));
		addMetadata(funTplNames, ColumnMetadata.named("FUN_TPL_NAMES").withIndex(7).ofType(Types.VARCHAR));
		...
		addMetadata(tplId, ColumnMetadata.named("TPL_ID").withIndex(12).ofType(Types.VARCHAR).withSize(32));
	}
