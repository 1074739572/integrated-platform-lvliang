...
public class QTSysConfig extends com.querydsl.sql.RelationalPathBase<TSysConfig> {
    ...
    public void addMetadata() {
    	...
    	addMetadata(endpointUrl, ColumnMetadata.named("ENDPOINT_URL").withIndex(10).ofType(Types.VARCHAR).withSize(128).notNull());
    	addMetadata(namespaceUrl, ColumnMetadata.named("NAMESPACE_URL").withIndex(11).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(databaseName, ColumnMetadata.named("DATABASE_NAME").withIndex(12).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(databaseUrl, ColumnMetadata.named("DATABASE_URL").withIndex(13).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(databaseDriver, ColumnMetadata.named("DATABASE_DRIVER").withIndex(14).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(driverUrl, ColumnMetadata.named("DRIVER_URL").withIndex(15).ofType(Types.VARCHAR).withSize(128));
        addMetadata(jsonParams, ColumnMetadata.named("JSON_PARAMS").withIndex(16).ofType(Types.LONGVARCHAR).withSize(65535));
        addMetadata(userName, ColumnMetadata.named("USER_NAME").withIndex(17).ofType(Types.VARCHAR).withSize(32).notNull());
        addMetadata(userPassword, ColumnMetadata.named("USER_PASSWORD").withIndex(18).ofType(Types.VARCHAR).withSize(128).notNull());
        addMetadata(createdBy, ColumnMetadata.named("CREATED_BY").withIndex(19).ofType(Types.VARCHAR).withSize(32).notNull());
