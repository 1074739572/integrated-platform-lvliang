...
//		String yyyyMM = new SimpleDateFormat("yyyyMM").format(date);
    ...
    public static Date stringToDate(String timeStr, String format) {
        ...
        if (StringUtils.isNotEmpty(timeStr)) {
            try {
                ...
                logger.error("string to date fail", e);
                return null;
            }
        }
        return null;
    ...
    public static long getDiffSeconds(Date startTime, Date endTime) {
        ...
        if (endTime == null) {
            endTime = getOfDayLast(startTime);
        }
        long diffMills = endTime.getTime() - startTime.getTime();

        return Math.abs(diffMills) / 1000;
    }

    public static long getTodayDiffSecondsFromNowToEnd() {
        return getDiffSeconds(null, null);
    }
}
