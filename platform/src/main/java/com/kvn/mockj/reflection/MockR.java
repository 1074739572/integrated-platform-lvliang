...
public class MockR {
    ...
    public static <T> T random(TypeReference<T> typeReference) {
        ...
        if (Collection.class.isAssignableFrom(targetClass)) {
            ...
            // Collection 下面只实例化一级对象，不做多级实例化
            // fix: 自己嵌套自己时，出现 StackOverflow。比如：Foo 中包含属性 List<Foo>
            return (T) randomCollection((ParameterizedTypeImpl) typeReference.getType(), 1);
        }
     ...
     * @return
     */
    private static Object randomCollection(ParameterizedTypeImpl targetType, int cascadeLayers){
        if (cascadeLayers <= 0) {
            return null;
        }
        ...
        Object obj = randomObject((Class) argType, cascadeLayers);
        instance.add(obj);
        return instance;
    }
